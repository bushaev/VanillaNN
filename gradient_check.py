...
def check_dense(epsilon=1e-9, error_epsilon=1e-6):
    ...
    for lind in range(2):
        ...
        for ind in range(net.layers[lind].bias.param.shape[0]):
            ...
            net.layers[lind].bias.param[ind, 0] -= 2 * epsilon
        ...
        for i, j in zip(range(net.layers[lind].weight.param.shape[0]),
                        range(net.layers[lind].weight.param.shape[1])):
            net.layers[lind].weight.param[i, j] += epsilon
            cost_plus = evaluate_cost(net, X, y)

            net.layers[lind].weight.param[i, j] -= 2 * epsilon
            cost_minus = evaluate_cost(net, X, y)
